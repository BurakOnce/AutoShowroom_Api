...
namespace AutoShowroom_UI.Controllers
{
    public class ServiceController : Controller
    {
        ...
        public async Task<IActionResult> CreateService(CreateServiceDto createServiceDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:44337/api/Services", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var reponseMessage = await client.DeleteAsync($"https://localhost:44337/api/Services/{id}");
